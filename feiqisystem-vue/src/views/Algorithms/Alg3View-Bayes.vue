...
<template>
    <div>
        <div>
            <div align="center"><span style="font-size: 20px">贝叶斯算法</span></div>
            <el-card shadow="hover" class="my-card">
                <div class="chart-container" style="width: 2000px;">
                    ...
                    <img src="/bayes/val_confusion_matrix.png" alt="" style="width: 30%;height: 500px;margin-left: 300px">
                    <div id="testreport" style="width: 30%;height: 500px;margin-left: 135px;margin-top: 30px"></div>
                    <img src="/bayes/test_confusion_matrix.png" alt="" style="width: 30%;height: 500px;margin-left: 300px">
                </div>
            </el-card>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        readDataFromFile2() {
            // 使用fetch API来读取本地文本数据
            fetch('/bayes/TestNaiveBayes.txt')
                    .then(response => response.text())
                    .then(textData => {
                        let lines = textData.split('\n');
                        lines.shift(); // 跳过标题行
